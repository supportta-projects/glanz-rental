-- Fix: Update process_order_return_optimized to correctly calculate missing items
-- When returned_quantity < quantity, those items should be considered as having missing items
-- This fixes the issue where partial returns (e.g., 40/45 items) weren't being handled correctly
...
BEGIN
  ...
  -- Step 4: Calculate new order status (single query) - FIXED: Check for missing items correctly
  ...
  IF EXISTS (
    ...
    -- FIX: Missing items = items where returned_quantity < quantity OR return_status = 'missing'
    SELECT 
...
-- Update comment
COMMENT ON FUNCTION process_order_return_optimized IS 'Optimized function to process order returns in single transaction. Handles partial returns, damage fees, and descriptions. FIXED: Correctly calculates missing items when returned_quantity < quantity.';
